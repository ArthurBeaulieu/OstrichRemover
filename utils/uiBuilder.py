from utils.tools import convertBytes, getTopicStringFromErrorCode
...
def _printErroredTracksReport_aux(errorCode, trackTester, albumTester):
    ...
    if errorCode == 0: # ErrorCode 00 : Filename release artists doesn't match the artist foldername
        printTrackErrorInfo(errorCode, track.fileNameList[0], track.fileNameList[0], trackTester.track.pathList[len(trackTester.track.pathList) - 2])
        ...
        printTrackErrorInfo(errorCode, track.fileNameList[4], track.artists)
...
def printTrackErrorInfo(errorCode, string1, string2):
    ...
    elif errorCode == 3 or errorCode == 5 or errorCode == 7 or errorCode == 9 or errorCode == 10:
        location1 = 'From Filename      '
